...
The third run (`8d1e9e37-3e6e-4d26-800a-7c42dc84aa81`, correlation `6304ff0e-c792-427d-b373-958d9b1b5b04`) used the replacement allowlisted Actor `valig/linkedin-jobs-scraper` with a hard USD 0.02 run cap. The Actor run `mSH3UsXeks132mtWj` succeeded in 4.983 seconds and reported an exact USD 0.001 charge, but its Dataset `3VJJ4RLLkDRjKDpCK` contained zero rows. Its recorded input was `AI Consultant`, `Amsterdam`, past month, with an effective per-query limit of 10. The discovery cycle completed as `partial` in 68.541 seconds, received two Adzuna observations, deduplicated them to two canonical vacancies, and blocked both after provider-fallback verification returned `SOURCE_RATE_LIMITED`. Eight SerpApi fallback attempts independently timed out at the 12-second bound. Shadow comparison recorded two legacy/search results, two multi-source results, and two overlaps, with zero duplicates and zero verified vacancies.
