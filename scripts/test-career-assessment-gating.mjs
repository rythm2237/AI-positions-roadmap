...
assert.match(bank, /stage\.resources\.map\(\(course,\s*courseIndex\)/);
assert.match(bank, /topicAssessments:\s*stage\.resources\.map/);
assert.match(bank, /course\.id/);
assert.match(bank, /assessmentScopeId\s*=\s*`\$\{stageId\}-course-\$\{topicIndex \+ 1\}`/);
assert.match(bank, /topicId\s*=\s*courseId\s*\?\?\s*assessmentScopeId/);
assert.match(bank, /id:\s*`\$\{assessmentScopeId\}-assessment`/);
assert.match(bank, /stage\.resources\.map\(\(course\) => course\.title\)\.join\(", "\)/);
...
assert.match(learning, /current\.topicAssessments[\s\S]*\.find\(/);
assert.match(learning, /item\.topicId\s*===\s*resource\.id/);
assert.match(learning, /onOpenNote\([\s\S]*"resource"/);
assert.doesNotMatch(learning, /15-question bank/);
assert.doesNotMatch(learning, /5 of 15 questions/);
