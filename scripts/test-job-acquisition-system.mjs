...
test("Layer 19 — automation honors timezone, report hour and channel", () => { const due = jobReportDue(agent, new Date("2026-09-03T18:15:00.000Z")); assert.equal(due?.type, "daily"); assert.equal(jobReportDue({ ...agent, notification_channels: ["in_app"] }, new Date("2026-09-03T18:15:00.000Z")), null); assert.match(source("vercel.json"), /0 \* \* \* \*/); });
