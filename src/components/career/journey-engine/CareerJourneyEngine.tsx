  ...
  return (
    ...
    >
      <span className="pointer-events-none absolute bottom-[78%] left-1/2 z-30 hidden w-64 -translate-x-1/2 flex-col items-center group-hover:flex group-focus-visible:flex">
        {(stage.title.match(/.{1,22}(?:\s|$)/g) ?? [stage.title]).map((line, index) => <span key={`${line}-${index}`} className="journey-hover-line journey-handwriting block max-w-full overflow-hidden whitespace-nowrap text-lg font-semibold leading-6 text-[#3f382d]" style={{ animationDelay: `${index * 110}ms` }}>{line.trim()}</span>)}
      </span>
      ...
      <span className={`journey-handwriting -mt-1 max-w-40 px-1 text-[11px] font-semibold leading-tight tracking-wide text-[#4d4a44] ${guidedMode && !active ? "opacity-0" : "opacity-100"}`}>{stage.label ?? stage.title}</span>
...
function GuidedOverlay({ stage, index, total, canOpen, travelling, onChange, onSelect, onExit }: {
  stage: CareerJourneyStage; index: number; total: number; canOpen: boolean; travelling: boolean;
  onChange: (index: number) => void; onSelect: () => void; onExit: () => void;
}) {
  ...
  return <>
    <motion.div className="pointer-events-none absolute left-1/2 top-[max(0.75rem,env(safe-area-inset-top))] z-40 w-[min(34rem,calc(100%-1.5rem))] -translate-x-1/2 px-4 py-2 text-center text-[#49463f]" initial={{opacity:0,y:-5}} animate={{opacity:1,y:0}}>
    ...
    <motion.div className="scrollbar-hide absolute bottom-[calc(4.75rem_+_env(safe-area-inset-bottom))] left-1/2 z-40 flex w-[min(34rem,calc(100%-1rem))] -translate-x-1/2 items-center justify-center gap-3 overflow-x-auto bg-[#eee7d5]/78 px-3 backdrop-blur-[2px] lg:bottom-4" initial={false} animate={{opacity: travelling ? .35 : 1}} transition={{duration:.18}}>
      ...
      <span className="hidden" aria-hidden="true">{canOpen ? "" : "Locked"}</span>
...
export default function CareerJourneyEngine(props: JourneyEngineProps) {
  const { map, stages, progress, viewport, focusedStage, selectedStage, guidedMode, guidedIndex, cameraPhase, learningMode, reduceMotion, dataWarnings, isStageUnlocked, getStageProgress, onSelectStage, onStartJourney, onExitJourney, onGuidedIndexChange } = props;
  ...
  const movePan = (x: number, y: number) => setPan({ x: Math.max(-world.width * camera.scale * .75, Math.min(viewport.width * .75, x)), y: Math.max(-world.height * camera.scale * .75, Math.min(viewport.height * .75, y)) });
  ...
  React.useEffect(() => {
    ...
    const endTimer = window.setTimeout(() => setTravelling(false), mobile ? 700 : 880);
  ...
  return (
    <motion.section className={`relative h-full touch-none select-none overflow-hidden bg-[#d8c69e] ${dragging ? "cursor-grabbing" : "cursor-grab"}`} initial={false} animate={{opacity:1}} exit={{opacity:0}} tabIndex={-1}
      ...
      <div className="absolute inset-0">
        <div className="absolute left-0 top-0 origin-top-left will-change-transform" style={{ width: world.width, height: world.height, transform: `translate3d(${pan.x}px,${pan.y}px,0) ${camera.transform}`, transition: reduceMotion || dragging ? "none" : `transform ${camera.transitionMs}ms cubic-bezier(.22,1,.36,1)` }}>
      ...
      {!guidedMode && showStartPopup ? <motion.div className="absolute inset-0 z-40 grid place-items-center px-4" initial={reduceMotion ? false : {opacity:0,scale:.97}} animate={{opacity:1,scale:1}}>
        <section role="dialog" aria-modal="true" aria-labelledby="journey-start-title" aria-describedby="journey-start-description" className="w-full max-w-lg rounded-3xl border border-[#f3d79b]/35 bg-[#5f4029]/78 p-6 text-center text-[#fff4dd] shadow-[0_24px_80px_rgba(57,32,17,.34),inset_0_1px_rgba(255,244,215,.18)] backdrop-blur-md sm:p-8">
          ...
          <h2 id="journey-start-title" className="journey-handwriting mt-3 text-2xl font-semibold sm:text-3xl">Zero to Employment Expedition</h2>
      ...
      </motion.div> : !guidedMode ? null : <GuidedOverlay stage={presentedStage} index={presentedIndex} total={stages.length} canOpen={isStageUnlocked(presentedStage.id)} travelling={travelling} onChange={(index) => { setPan({x:0,y:0}); onGuidedIndexChange(index); }} onSelect={() => onSelectStage(presentedStage)} onExit={() => { setPan({x:0,y:0}); onExitJourney(); }}/>} 
