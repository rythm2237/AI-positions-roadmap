...
export default function Header() {
  ...
  const [careerSearchOpen, setCareerSearchOpen] = useState(false);

  useEffect(() => {
    document.body.style.overflow = mobileOpen || overlay || careerSearchOpen ? "hidden" : "";
    return () => { document.body.style.overflow = ""; };
  }, [mobileOpen, overlay, careerSearchOpen]);
    ...
    <>
        ...
        <div className="mx-auto grid h-[68px] max-w-7xl grid-cols-[1fr_auto_1fr] items-center px-5 sm:px-8">
          ...
          <nav className="col-start-2 hidden items-center justify-center gap-1 xl:flex" aria-label="Primary navigation">
            ...
            {NAV_ITEMS.map((item) => item.kind === "link" ? (
              <Link key={item.label} href={item.href} className="whitespace-nowrap rounded-xl px-3 py-2 text-sm font-medium text-slate-400 transition hover:bg-white/[0.045] hover:text-white">{item.label}</Link>
            ) : (
              <button key={item.label} type="button" onClick={() => setOverlay(item.overlay)} className="whitespace-nowrap rounded-xl px-3 py-2 text-sm font-medium text-slate-400 transition hover:bg-white/[0.045] hover:text-white">{item.label}</button>
            ))}
          </nav>
      ...
      <aside className={`fixed inset-y-0 right-0 z-[71] flex w-[min(340px,88vw)] flex-col overflow-y-auto overscroll-contain border-l border-white/10 bg-[#070919]/98 p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "translate-x-full"}`} aria-hidden={!mobileOpen}>
        ...
        <nav className="mt-5 grid gap-2" aria-label="Mobile primary navigation">
          <button type="button" onClick={() => { setMobileOpen(false); setCareerSearchOpen(true); }} className="rounded-xl border border-cyan-300/15 bg-cyan-300/[0.06] px-4 py-3 text-left text-sm font-semibold text-cyan-100">Search Careers<span className="float-right">⌕</span></button>
          {NAV_ITEMS.map((item) => item.kind === "link" ? (
      ...
      <RoleSearchDialog open={careerSearchOpen} onClose={() => setCareerSearchOpen(false)} />
    </>
  );
}
