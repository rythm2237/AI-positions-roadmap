"use client";

import dynamic from "next/dynamic";
import Link from "next/link";
import { Component, type ErrorInfo, type ReactNode, useEffect, useState } from "react";

type SupportState = "checking" | "supported" | "unsupported";

function canCreateWebGLContext() {
  try {
    const canvas = document.createElement("canvas");
    return Boolean(
      canvas.getContext("webgl2", { failIfMajorPerformanceCaveat: true }) ||
        canvas.getContext("webgl", { failIfMajorPerformanceCaveat: true }) ||
        canvas.getContext("experimental-webgl"),
    );
  } catch {
    return false;
  }
}

function UniverseFallback({ checking = false }: { checking?: boolean }) {
  return (
    <div className="relative flex h-dvh min-h-[100svh] w-full items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_50%_15%,rgba(124,58,237,.2),transparent_42%),linear-gradient(180deg,#080b1c,#03050e)] px-5 text-center sm:px-6">
      <div className="absolute inset-0 opacity-40 [background-image:linear-gradient(rgba(255,255,255,.035)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.035)_1px,transparent_1px)] [background-size:44px_44px]" aria-hidden="true" />
      <div className="relative max-w-2xl pt-10 sm:pt-16">
        <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl border border-violet-300/20 bg-violet-500/10 text-violet-200 shadow-[0_0_45px_rgba(124,58,237,.25)] sm:h-16 sm:w-16" aria-hidden="true">
          <svg viewBox="0 0 24 24" className="h-7 w-7 sm:h-8 sm:w-8" fill="none" stroke="currentColor" strokeWidth="1.5">
            <path d="M4 17 9 12l3 3 8-8" />
            <path d="M15 7h5v5" />
          </svg>
        </div>
        <p className="mt-5 text-[10px] font-semibold uppercase tracking-[.2em] text-violet-300 sm:mt-6 sm:text-xs">Career Universe</p>
        <h1 id="homepage-title" className="mt-3 font-display text-[clamp(2.25rem,10vw,3.75rem)] font-bold leading-[1.02] tracking-tight text-white">
          Build your career in AI.
        </h1>
        <p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-slate-400 sm:text-base sm:leading-7">
          {checking
            ? "Preparing the interactive Career Universe. You can open the complete career directory immediately if you prefer a standard list view."
            : "This device cannot create a stable WebGL context. The complete career directory and every public roadmap remain available."}
        </p>
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3 sm:mt-7">
          <Link href="/careers" className="inline-flex min-h-11 items-center justify-center rounded-xl bg-white px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-violet-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400">
            Explore AI Careers
          </Link>
          {!checking && (
            <Link href="/login" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/[0.08] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400">
              Sign in
            </Link>
          )}
        </div>
      </div>
    </div>
  );
}

const OpeningScene = dynamic(() => import("@/components/opening-scene/OpeningScene"), {
  ssr: false,
  loading: () => <UniverseFallback checking />,
});

class UniverseErrorBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  state = { failed: false };

  static getDerivedStateFromError() {
    return { failed: true };
  }

  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error("Career Universe failed to render safely", error, info.componentStack);
  }

  render() {
    if (this.state.failed) return <UniverseFallback />;
    return this.props.children;
  }
}

export default function SafeCareerUniverse() {
  const [support, setSupport] = useState<SupportState>("checking");

  useEffect(() => {
    setSupport(canCreateWebGLContext() ? "supported" : "unsupported");
  }, []);

  if (support === "checking") return <UniverseFallback checking />;
  if (support === "unsupported") return <UniverseFallback />;

  return (
    <UniverseErrorBoundary>
      <div className="h-dvh min-h-[100svh] w-full overflow-hidden bg-[#03050e]">
        <OpeningScene />
      </div>
    </UniverseErrorBoundary>
  );
}
