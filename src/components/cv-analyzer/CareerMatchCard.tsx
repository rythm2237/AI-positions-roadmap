  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-white/[0.025] p-4">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500">
        ...
        <span>Relevant duration: {match.professionalEvidence.durationBucket}</span>
      </div>
      ...
      <details className="mt-4 rounded-xl border border-white/8 bg-black/10 px-3 py-2.5">
        ...
        <div className="mt-3 grid gap-3 text-xs leading-5">
          ...
          <section><h4 className="font-semibold text-amber-200/85">Limiting factors</h4><ul className="mt-1 text-slate-400">{match.evidenceSummary.limitingFactors.map((signal) => <li key={signal}>• {signal}</li>)}</ul></section>
        </div>
      </details>

      <p className="mt-3 text-xs leading-5 text-slate-500">Gap-closing estimate: {match.weeks}</p>
    </article>
  );
}
