...
function requiredLanguages(text: string) { const found = new Set<string>(); for (const sentence of text.split(/(?<=[.!?;:\n])\s+/).filter(Boolean)) { if (!requirementWords.test(sentence)) continue; for (const [language, pattern] of languagePatterns) if (pattern.test(sentence)) found.add(language); } return [...found]; }
function inferPostingLanguage(text: string): string | null { const lower = text.toLowerCase(); const de = lower.match(/\b(und|oder|mit|für|fuer|wir|sie|der|die|das|eine|einen|deine|ihre|aufgaben|anforderungen|kenntnisse|berufserfahrung|arbeitszeit|arbeitsplatz|bewerbung)\b/g)?.length ?? 0; const en = lower.match(/\b(and|or|with|for|we|you|the|your|responsibilities|requirements|experience|skills|working|workplace|application)\b/g)?.length ?? 0; const fr = lower.match(/\b(et|avec|pour|nous|vous|les|des|une|compétences|competences|expérience|experience|candidature|poste)\b/g)?.length ?? 0; if (de >= 5 && de >= en * 1.3) return "German"; if (fr >= 5 && fr >= en * 1.3) return "French"; if (en >= 5) return "English"; return null; }
const normalizeLanguage = (value: string) => value.trim().toLowerCase();
function hasLanguage(profile: Profile, language: string) { const target = normalizeLanguage(language); return profile.languages.some((item) => { const known = normalizeLanguage(item); return known.includes(target) || target.includes(known); }); }
...
export async function runJobSearch() {
  ...
  await supabase.from("user_activity").insert({ user_id: user.id, action: "job_agent_search_run", metadata: { reviewed: rows.length, providers: ["Adzuna"], language_gate: true, expanded_role_queries: roleQueries } });
  revalidatePath("/job-agent"); redirect(`/job-agent?searched=${rows.length}&expanded=${roleQueries.length}`);
}
