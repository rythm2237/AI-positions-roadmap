...
export default async function ApplicationPackPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ generated?: string }> }) {
  ...
  const [applicationResult, assetsResult] = await Promise.all([
    supabase.from("applications").select("id,status,agent_mode,next_action,notes,created_at,job_opportunities(id,company,role,location,job_url,fit_score,source,founder_positioning,status,recommendation,skip_reason)").eq("id", id).eq("user_id", user.id).maybeSingle(),
  ...
  const stale = job?.status === "skipped" || job?.recommendation === "skip";
  ...
  return <main className="mx-auto max-w-5xl px-5 py-10 sm:py-12">
    ...
    {stale ? <div className="mt-6 rounded-2xl border border-amber-300/20 bg-amber-400/[.05] p-4"><p className="text-xs font-semibold uppercase tracking-[.14em] text-amber-200">No longer matches current criteria</p><p className="mt-2 text-sm leading-6 text-slate-300">This application is preserved as history, but it is no longer an active recommendation under your current country/language/role settings. {job?.skip_reason ?? "Run a new search to get current eligible jobs."}</p></div> : application.next_action ? <div className="mt-6 rounded-2xl border border-amber-300/15 bg-amber-400/[.04] p-4"><p className="text-xs font-semibold uppercase tracking-[.14em] text-amber-200">Next action</p><p className="mt-2 text-sm text-slate-300">{application.next_action}</p></div> : null}

    <div className="mt-7 grid gap-6 lg:grid-cols-2">
      <section className="glass rounded-2xl border border-white/[.07] p-5"><h2 className="font-display text-xl font-semibold text-white">Tailored CV content</h2>{summary?.text ? <div className="mt-4"><p className="text-xs uppercase tracking-[.14em] text-slate-500">Professional summary</p><p className="mt-2 text-sm leading-6 text-slate-300">{summary.text}</p></div> : null}{skills.length ? <div className="mt-4 flex flex-wrap gap-2">{skills.map((skill) => <span key={skill} className="rounded-full bg-cyan-400/[.07] px-2.5 py-1 text-xs text-cyan-100">{skill}</span>)}</div> : null}<div className="mt-5 space-y-3">{highlights.map((item, index) => <div key={`${item.text}-${index}`} className="rounded-xl border border-white/[.06] p-3"><p className="text-sm leading-6 text-slate-300">{item.text}</p><p className="mt-2 text-[11px] text-slate-600">Evidence: {item.evidenceIds?.join(", ")}</p></div>)}</div>{!hasPack ? <p className="mt-4 text-sm text-slate-500">No generated CV content is stored for this application.</p> : null}</section>
      ...
      <section className="glass rounded-2xl border border-white/[.07] p-5"><h2 className="font-display text-xl font-semibold text-white">Cover / application note</h2><div className="mt-4 space-y-4">{paragraphs.map((item, index) => <p key={index} className="text-sm leading-6 text-slate-300">{item.text}</p>)}{!paragraphs.length ? <p className="text-sm text-slate-500">No cover note is stored for this application.</p> : null}</div></section>
    ...
    <div className="mt-7 flex flex-col gap-3 sm:flex-row">{stale ? <Link href="/job-agent" className="btn-primary inline-flex min-h-11 items-center justify-center">Find current eligible jobs</Link> : !hasPack && job?.id ? <form action={prepareApplication}><input type="hidden" name="job_id" value={job.id} /><button className="btn-primary min-h-11">Retry application pack</button></form> : <a href={job?.job_url ?? "#"} target="_blank" rel="noreferrer" className="btn-primary inline-flex min-h-11 items-center justify-center">Continue to vacancy</a>}<p className="self-center text-xs leading-5 text-slate-500">A pack is <strong className="text-slate-300">Ready for Review</strong> only after successful generation. “Applied” and “Ready for Submit” require actual external execution evidence.</p></div>
  </main>;
}
