...
export async function saveJobAgent(form: FormData) {
  ...
  const payload = {
    ...
    search_countries: list(form, "search_countries"),
    excluded_countries: list(form, "excluded_countries"),
    ...
    relocation_countries: list(form, "relocation_countries"),
  ...
  if (profileLanguages.length) {
    await supabase.from("profiles").update({ languages: profileLanguages }).eq("id", user.id);
  }
  await supabase.from("user_activity").insert({ user_id: user.id, action: "job_agent_configured", metadata: { mode: automationMode } });
  revalidatePath("/job-agent");
  redirect("/job-agent?saved=1");
}
