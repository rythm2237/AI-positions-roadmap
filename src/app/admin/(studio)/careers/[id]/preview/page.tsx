...
export default async function CareerPreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><aside className="border-b border-violet-300/20 bg-[#0b0c1b] px-4 py-3 text-slate-200 sm:px-8"><div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold">Admin Draft Preview · {career.title}</p><p className="mt-1 text-xs text-slate-500">{publication.valid ? "Publication checks passed." : `${publication.errors.length} publication check${publication.errors.length === 1 ? "" : "s"} remain; preview is still available for review.`}</p></div><div className="flex flex-wrap gap-3"><Link href={`/admin/careers/${id}/content`} className="text-sm font-semibold text-cyan-300 underline">Edit Blueprint</Link><Link href={`/admin/careers/${id}/resources`} className="text-sm font-semibold text-cyan-300 underline">Learning Sources</Link></div></div></aside><CareerWorkspace career={content.data}/></>;
}
