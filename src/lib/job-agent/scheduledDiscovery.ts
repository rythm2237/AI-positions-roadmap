...
export async function runScheduledJobDiscovery() {
  ...
  return executeScheduledJobDiscovery({
    ...
    async loadAgents(limit) {
      const response = await fetch(`${database.url}/rest/v1/job_agents?status=eq.active&select=user_id,updated_at&order=updated_at.asc&limit=${limit}`, { headers: { apikey: database.key, Authorization: `Bearer ${database.key}` }, cache: "no-store", signal: AbortSignal.timeout(10_000) });
