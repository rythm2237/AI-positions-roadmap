...
function localParts(date: Date, timezone: string) { const formatter = new Intl.DateTimeFormat("en-CA", { timeZone: timezone, year: "numeric", month: "2-digit", day: "2-digit", weekday: "short" }); const parts = Object.fromEntries(formatter.formatToParts(date).map((part) => [part.type, part.value])); return { date: `${parts.year}-${parts.month}-${parts.day}`, weekday: parts.weekday }; }
function due(agent: ServiceAgent, now: Date) { if (agent.status !== "active" || agent.report_frequency === "none" || !agent.notification_channels.includes("email")) return null; const local = localParts(now, agent.timezone || "UTC"); if (agent.report_frequency === "weekly" && local.weekday !== "Mon") return null; return { type: agent.report_frequency as "daily" | "weekly", periodKey: agent.report_frequency === "weekly" ? `${local.date}:week` : local.date }; }
...
export async function sendDueJobAgentReports(now = new Date()) {
  ...
  for (const { agent, schedule } of dueAgents) {
    ...
    const summary = { reviewed: userJobs.length, qualified: userJobs.filter((job) => (job.fit_score ?? 0) >= agent.auto_prepare_threshold).length, strongMatches: userJobs.filter((job) => (job.fit_score ?? 0) >= agent.strong_match_threshold).length, applied: userApps.filter((application) => application.status === "applied").length, readyForSubmit: userApps.filter((application) => application.status === "ready_for_submit").length, needsDecision: userJobs.length };
