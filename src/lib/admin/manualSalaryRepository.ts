...
export interface SalarySourceReadiness {id:string;country_code:string;source_name:string}
...
interface SupabaseErrorBody {code?:string;message?:string;details?:string;hint?:string}
async function read<T>(path:string,token:string,init:RequestInit={}){const response=await supabaseUserFetch(`/rest/v1/${path}`,token,{...init,headers:{Prefer:"return=representation",...init.headers}});if(!response.ok){const body=await response.json().catch(()=>({})) as SupabaseErrorBody;console.error("MANUAL_SALARY_DATABASE_ERROR",{resource:path.split("?")[0],status:response.status,code:body.code??null,message:body.message??null,details:body.details??null,hint:body.hint??null});throw new Error("MANUAL_SALARY_DATABASE_ERROR")}return response.json() as Promise<T>}
...
export function listSalarySourceReadiness(token:string){return read<SalarySourceReadiness[]>("statistical_sources?select=id,country_code,source_name&order=country_code.asc,source_name.asc",token)}
