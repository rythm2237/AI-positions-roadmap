...
export interface RetrievedRelease{providerReleaseId:string;releaseDate:string;referencePeriodStart:string;referencePeriodEnd:string;retrievedAt:string;sourceMetadata:Record<string,unknown>;records:unknown[]}
export interface OfficialSourceAdapter{id:string;countries:readonly string[];capabilities:readonly IntelligenceCapability[];plan(selection:OfficialRefreshSelection):Promise<AdapterPlan>;retrieve(item:AdapterPlanItem):Promise<RetrievedRelease>;normalize(item:AdapterPlanItem,release:RetrievedRelease):Promise<NormalizedObservationInput[]>}
